// src/components/Courses/MaterialChatPanel.jsx
// Right-side slide-in drawer that lets a course owner or enrolled student
// chat with a single CourseMaterial (lecture-note PDF or course video).
// Streams answers from /api/material-chat/query/stream and renders citation
// chips that point back to the source page or timestamp.
...
const SUGGESTIONS_VIDEO = [
  'Give me a 5-bullet summary of this video.',
  'What is the most important takeaway?',
  'Generate three quiz questions from this lecture.',
];

// ── Citation chip ───────────────────────────────────────────────────────
const CitationChip = ({ citation, materialType, onJump }) => {
  let label = null;
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md
                 bg-gray-900 border border-gray-700 hover:border-teal-500/50
                 text-[11px] text-gray-300 hover:text-teal-300 transition-colors"
      type="button"
    >
...
const MessageRow = ({ msg, materialType, onJumpCitation }) => {
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1 text-sm text-gray-100 leading-relaxed">
          {msg.content
            ? parseMarkdownWithMath(msg.content)
            : <ThinkingDots />}
        </div>
      </div>
...
// ── Main panel ──────────────────────────────────────────────────────────
const MaterialChatPanel = ({ material, onClose, onJumpCitation }) => {
  ...
  const isPdf = materialType === 'lecture_notes';
  ...
  // ── Load existing sessions when the panel opens ──────────────────────
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setSessions(rows);
        // Auto-resume the most recent session, if any.
        if (rows.length > 0 && !activeSessionId) {
          await loadSession(rows[0].id);
        }
      } catch (e) {
        // 403 (not-enrolled) and 404 are handled by the indexing-state gate
        // above; everything else is a soft failure.
      }
    })();
  ...
  const submit = async (queryText) => {
    ...
    try {
      ...
      if (resolvedSessionId && resolvedSessionId !== activeSessionId) {
        ...
        // Refresh the side list so the new session shows up at the top.
        try {
          const rows = await materialChatApi.listSessions(material.id);
          setSessions(rows);
        } catch {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex">
      ...
      <aside
        role="dialog"
        aria-label="Chat with material"
        className={`relative ml-auto h-full w-full sm:w-[560px] md:w-[640px] lg:w-[720px]
                    bg-gray-950 border-l border-gray-800 shadow-2xl shadow-black/40
                    flex flex-col
                    transition-transform duration-200 ease-out
                    ${mounted ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* ── Header ───────────────────────────────────────────────────── */}
        <div className="flex items-start gap-3 px-5 py-4 border-b border-gray-800/80 bg-gradient-to-b from-gray-900/60 to-transparent">
          <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center
                          ${isVideo
                            ? 'bg-purple-500/15 border border-purple-500/30'
                            : 'bg-blue-500/15 border border-blue-500/30'}`}>
            <HeaderIcon size={16} className={isVideo ? 'text-purple-300' : 'text-blue-300'} />
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 text-[11px] uppercase tracking-[0.14em] text-teal-400/80">
              <Sparkles size={11} />
          ...
          <button
            onClick={handleClose}
            className="flex-shrink-0 p-1.5 text-gray-500 hover:text-white transition-colors rounded-md hover:bg-gray-800"
            title="Close (Esc)"
            type="button"
          >
            <X size={16} />
        ...
        {/* ── Session bar ──────────────────────────────────────────────── */}
        <div className="flex items-center gap-2 px-5 py-2.5 border-b border-gray-800/80 bg-gray-900/30">
          <button
          ...
          {activeSessionId && (
            <span className="ml-auto text-[11px] text-gray-500 truncate max-w-[200px]">
        ...
        {sessionsOpen && (
          <div className="border-b border-gray-800 max-h-56 overflow-y-auto bg-gray-900/40">
            {sessions.length === 0 ? (
              <div className="px-5 py-4 text-xs text-gray-500">No previous chats yet.</div>
            ) : (
              sessions.map((s) => (
                <button
                  key={s.id}
...
export default MaterialChatPanel;
