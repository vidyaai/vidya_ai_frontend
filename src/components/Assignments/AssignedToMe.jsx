...
const AssignedToMe = ({ onBack = undefined, onNavigateToHome = undefined, initialCourseId, initialSection, embed = false }) => {
  ...
  const getStudentStatus = (assignment) => {
    ...
    switch (status) {
      ...
      case 'submitted':
        ...
        color = 'bg-[#43ead6]/15 text-[#43ead6]';
        break;
      case 'graded':
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default:
        return 'bg-[#43ead6]/15 text-[#43ead6]';
    }
  };
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      default:
        return <Clock size={16} className="text-[#43ead6]" />;
    }
  };
      ...
      {showOpenAssignments && (
            ...
            <div className="bg-[#0d1f38] rounded-xl p-6 border border-[#182842]">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-slate-400">Total Assigned</p>
                </div>
              </div>
            </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            <Loader2 size={32} className="text-[#43ead6] animate-spin" />
        ...
        {!loading && !error && assignedAssignments.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 mb-6">You don't have any assignments assigned to you yet.</p>
          </div>
        )}
            ...
            {assignedAssignments.map((sharedAssignment) => {
              ...
              return (
                <div
                  ...
                  className="bg-[#0d1f38] rounded-xl p-6 border border-[#182842] hover:border-[#1a2943] transition-all duration-300"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-white mb-2">{assignment.title}</h3>
                      <p className="text-slate-400 text-sm mb-3 line-clamp-3">{assignment.description}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-center mb-4">
                    <div>
                      <div className="text-2xl font-bold text-[#43ead6]">{assignment.total_questions}</div>
                      <div className="text-xs text-slate-500">Questions</div>
                    </div>
                    <div>
                      <div className="text-2xl font-bold text-[#43ead6]">{assignment.total_points}</div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {(assignment.question_types || []).map((type, index) => (
                      <span
                        key={index}
                        className="px-2 py-1 bg-white/[0.08] text-slate-300 text-xs rounded"
                      >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#1a2943]">
                      ...
                      className={`px-4 py-2 text-white rounded-lg transition-all duration-300 ${
                        (() => {
                          ...
                          if (studentStatus.status === 'graded') {
                            ...
                            return 'bg-gradient-to-r bg-[#43ead6] hover:bg-[#43ead6]/90';
