  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const generateQuestions = () => {
        ...
        if (generationOptions.perQuestionDifficulty) {
          // Use specific difficulty distribution
          let questionId = 1;
          
          Object.entries(generationOptions.difficultyDistribution).forEach(([difficulty, config]) => {
            if (generationOptions.setCustomPoints && generationOptions.pointsVariation === 'varying') {
          ...
          // Use original logic for uniform difficulty
          const numQuestions = parseInt(generationOptions.numQuestions);
          const pointsPerQuestion = Math.floor(generationOptions.totalPoints / numQuestions);
          
          for (let i = 1; i <= numQuestions; i++) {
            let difficulty;
            let points;
            
            if (generationOptions.difficultyLevel === 'mixed') {
              difficulty = difficulties[Math.floor(Math.random() * difficulties.length)];
            } else {
              difficulty = generationOptions.difficultyLevel;
            }
            
            // Set points based on difficulty level
            if (generationOptions.setCustomPoints) {
              points = generationOptions.difficultyDistribution[difficulty].pointsEach;
            } else {
              // Use calculated points per question from total points
      ...
      const totalPoints = generationOptions.perQuestionDifficulty 
