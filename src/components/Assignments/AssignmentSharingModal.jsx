    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white/5 border border-[#1a2943] rounded-lg p-4 sm:p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white rounded flex-shrink-0 ml-2">
            <X size={20} />
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {existingLinkId && (
              <div className="bg-[#43ead6]/10 bg-opacity-50 border border-blue-700 rounded-lg p-4">
                <div className="flex items-center gap-2 text-[#43ead6] mb-2">
                  <Share2 size={16} />
                  <span className="font-medium">Editing Existing Share Link</span>
                </div>
                <p className="text-[#43ead6] text-sm">You're editing an existing share link.</p>
              </div>
            )}

            {/* Assignment Details */}
            <div className="bg-[#0d1f38] rounded-lg p-4 border border-[#1a2943]">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  {assignment.description && <p className="text-slate-400 text-sm mb-3">{assignment.description}</p>}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3">
                {/* PDF */}
                <div className="bg-[#0d1f38] rounded-lg p-4 border border-[#1a2943] flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="p-2 bg-red-500/10 rounded-lg"><FileText className="text-red-400" size={18} /></div>
                    <div><div className="font-medium text-white">PDF Document</div><div className="text-sm text-slate-400">Download formatted PDF</div></div>
                  </div>
                  <button onClick={handleDownloadPDF} disabled={isDownloadingPDF} className="px-4 py-2 bg-[#43ead6] hover:bg-[#43ead6]/90 disabled:bg-[#43ead6]/30 text-white rounded-lg transition-colors flex items-center space-x-2 text-sm">
            ...
            {/* Privacy Settings */}
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-3">Privacy Settings</label>
              <div className="space-y-3">
            ...
            {!isPublic && (
              <div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-lg border text-sm transition-colors ${
                        shareWithCourse
                          ...
                          : 'bg-[#0d1f38] border-[#1a2943] text-slate-300 hover:border-white/20'
                      }`}
                    >
                ...
                <label className="block text-sm font-medium text-slate-300 mb-3">Invite Students</label>
                
                {existingUsers.length > 0 && (
                  <div className="mb-3">
                    <label className="block text-sm font-medium text-slate-400 mb-2">Currently Invited ({existingUsers.length})</label>
                    <div className="flex flex-wrap gap-2">
                      {existingUsers.map(user => (
                        <div key={user.uid} className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm ${user.isPending ? 'bg-yellow-600/20 border border-yellow-500/50 text-yellow-300' : 'bg-white/10 text-white'}`}>
                          <span>{user.displayName || user.email}</span>
                          {user.isPending && <span className="text-xs opacity-75">(pending)</span>}
                          <button onClick={() => handleRemoveExistingUser(user.uid)} className={user.isPending ? 'text-yellow-200 hover:text-white' : 'text-slate-300 hover:text-white'} title="Remove"><X size={14} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {selectedUsers.length > 0 && (
                  <div className="mb-3">
                    <label className="block text-sm font-medium text-slate-400 mb-2">New Students to Invite</label>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && searchQuery && EMAIL_REGEX.test(searchQuery.trim())) { e.preventDefault(); handleAddEmail(searchQuery); } }} placeholder="Search by email or enter new email..." className="w-full pl-10 pr-10 py-2 bg-white/[0.08] border border-white/20 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500" />
                  {isSearching && <div className="absolute right-3 top-1/2 transform -translate-y-1/2"><Loader2 size={18} className="animate-spin text-slate-400" /></div>}
                </div>

                {searchQuery && (
                  <div className="mt-2 bg-white/[0.08] border border-white/20 rounded-lg max-h-40 overflow-y-auto">
                    {isSearching ? (
                      <div className="p-4 text-center text-slate-400"><Loader2 size={20} className="animate-spin mx-auto" /></div>
                    ) : searchResults.length > 0 ? (
                      searchResults.map(user => (
                        <button key={user.uid} onClick={() => handleSelectUser(user)} className="w-full px-3 py-2 text-left hover:bg-white/10 flex items-center gap-2 border-b border-white/20 last:border-b-0">
                          <UserPlus size={16} className="text-teal-400" />
                          <div><div className="text-white text-sm">{user.displayName || 'Unknown'}</div><div className="text-slate-400 text-xs">{user.email}</div></div>
                        </button>
                      ))
                    ) : EMAIL_REGEX.test(searchQuery.trim()) ? (
                      <button onClick={() => handleAddEmail(searchQuery)} className="w-full px-3 py-2 text-left hover:bg-white/10 flex items-center gap-2">
                        <UserPlus size={16} className="text-yellow-400" />
                        <div><div className="text-white text-sm">Add "{searchQuery.trim()}"</div><div className="text-slate-400 text-xs">Invite new user by email</div></div>
                      </button>
                    ) : (
                      <div className="p-3 text-center text-slate-400 text-sm">{searchQuery.includes('@') ? 'Enter a valid email address' : 'No users found. Enter full email to invite.'}</div>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <label htmlFor="csv-upload" className="flex items-center gap-2 px-4 py-2 bg-white/[0.08] hover:bg-white/10 text-white rounded-lg cursor-pointer transition-colors text-sm">
                    {isProcessingCsv ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}
                    <span>{isProcessingCsv ? 'Processing...' : 'Upload CSV'}</span>
                  </label>
                  <span className="text-xs text-slate-400">Upload CSV with student emails</span>
                </div>
            ...
            <div className="flex gap-3 pt-4">
              <button onClick={onClose} className="flex-1 px-4 py-2 bg-white/[0.08] hover:bg-white/10 text-white rounded-lg transition-colors">Cancel</button>
              {existingLinkId && (
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="text" value={getShareUrl()} readOnly className="flex-1 px-3 py-2 bg-white/[0.08] border border-white/20 rounded-lg text-white text-sm" />
            ...
            <div className="bg-white/[0.08] rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-400">Assignment:</span><span className="text-white">{assignment.title}</span></div>
                ...
                {!shareLinkData.is_public && existingUsers.length > 0 && (
                  <div>
                    <span className="text-slate-400">Invited Students:</span>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShareLinkData(null)} className="flex-1 px-4 py-2 bg-white/10 hover:bg-white/15 text-white rounded-lg transition-colors flex items-center justify-center gap-2">
