    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <svg className="animate-spin" width="80" height="80" viewBox="0 0 80 80">
          <defs>
            <mask id="crescent-mask-course-detail">
              <circle cx="40" cy="40" r="36" fill="white" />
              <circle cx="43" cy="40" r="37" fill="black" />
            </mask>
          </defs>
          <circle cx="40" cy="40" r="36" fill="white" mask="url(#crescent-mask-course-detail)" />
        </svg>
      </div>
    );
  }
...
const LectureNotesSection = ({ courseId }) => {
  ...
  // Poll every 5s while any uploaded note is still being indexed (chunked+embedded)
  useEffect(() => {
